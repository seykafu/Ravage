...
| 2026-09 | Ravage aura follows the sprite, not the tile | Reported: the red Ravage glow sits off the character — sometimes near, sometimes well away. Cause: the aura was placed from `projection.tileToWorld(u.state.position)` and only repositioned by `refreshUnitView`, which is event-driven. `moveUnit()` writes the DESTINATION into `state.position` before the walk tween starts, so the glow teleported to the destination tile and burned on empty ground for the whole walk — measured at 192-218px adrift mid-move, snapping back only once the unit arrived. The attack lunge had the same problem. Fixed by making the aura track the SPRITE: `syncRavageAura(view)` parks it at `sprite.y + AURA_FOOT_OFFSET` and `BattleScene.update()` calls it every frame for every view, so it follows walks, lunges and the idle bob alike. Verified in the running game — drift is 0px through a four-tile move and through an attack lunge, for a swordsman, an archer and a dactyl rider. No unit test: the suite is deliberately Phaser-free, and the defect was WHEN positioning ran rather than the arithmetic, which a pure test could not have caught. |
| 2026-08 | Fork snapshot audit — items in bags and lives were not captured | Asked to double-check that levels, items and lives all roll back to chapter 18 on an another-path run. Levels were correct. The other two were not. (1) The snapshot stored only `squadInventory` — the squad POOL — while the rewind clears `assignedInventory`, so every item still sitting in a character's bag was destroyed. Measured in the live game: a squad carrying 12 unused potions into B18 came back from the rewind with ZERO items. The snapshot now folds bags into the pool. (2) `squadDeaths` was never recorded at all, so the `...s` spread carried the FINISHED run's death count into the new road — a player who lost six units across twenty-eight chapters began a fresh ten-chapter road one casualty from Game Over. Now snapshotted and restored (missing value reads as 0, the generous and more accurate-at-the-fork default). (3) Capture also moved to the very end of the B18 victory sequence: it used to run before the rewards were minted and before `reconcilePostBattleInventory` settled the bags, so B18's own spoils (3 Elixirs + a Royal Lens) and anything salvaged off a fallen unit were missing from the snapshot too. Note equipment (Mask/Fang/Royal Lens/Dactyl Food) is `consumedOnBattleEnd` by design, so only potions and elixirs ever persist in bags — that is correct behaviour, not a leak. |
| 2026-08 | Endgame emotional pass — B28 beats, a longer aftermath, and a shared farewell | The last battle of an eleven-year story had two speaking characters in it and the credits started ninety seconds after the wedding. Added a mid-fight beat to each of the five roads (round 3), so the people who walked the whole campaign speak in the fight that ends it: Ranatoli on what the cell bought him, Selene telling Amar she never asked for the list, Maya making him LOOK at his mother, Corin on the order that had everyone's name on it, Leo asking for the kill shot and being told no. Extended `post_path_final` from 8 beats to 17 — the squad standing on the marble not believing it is over, Ning unable to lower the bow, and the gap in the line where Lucian is not standing. Extended all five `post_ending_*` codas before the marriage question. NEW: `where_they_went`, a shared closing arc that every wedding coda AND `end_alone` route through before the credits (the wedding closes Amar's story; this closes everyone else's) — a road each for Ning, Leo, Ranatoli, Veya, Corin, Selene, Ndara, Maya, Lucian's empty chair, and Khione. Written partner-agnostically: `StoryArc.beats` is static data with no save access, and every road has to read true whether or not Amar married that person. Exile and forgetting deliberately still go straight to credits — Amar walks away at B19 and there is no squad to say goodbye to. |
...
| 2026-05 | B12 (The Ravage / Grude harbor) authored — second half opens | First playable chapter past the Lucian-death pivot. Squad arrives at Grude's east port after the 14-month crossing; Captain Volos's interception detail meets them on the dock; Madame Dawn finishes the colony-truth speech Kian started at the cliffs. **Map:** new 14×11 cobblestone harbor district (`ravageMap`) — wooden dock at south for player spawn, marble customs platform at row 3 for the empire's officers, stone-warehouse walls on both sides forcing combat to a central corridor, north city gate as escape tile. **Units:** post-Lucian squad of four (Amar / Ning / Maya / Leo); enemies are 6 elite at level 12-13 anchored by new `ENEMIES.archboldCaptain` (named "Captain Volos") with `holdPositionUntil { allyCount: 2 }`. Reuses Kian's palette + portrait as a stand-in until Volos art ships. **Victory:** `anyOf(escapeToTile north, routEnemies)` — the cinematic intent is escape into the city, but a player who wants to clear the dock for full XP can. **Mid-battle dialogues (4):** `b12_dawn_voice_window` (round 1, Dawn shouting from a window: "It's the SAME KIT. Eighty years of it. You are not fighting a kingdom, my son. You are fighting an empire."); `b12_dawn_son_beat` (round 3, follow-up where Amar catches the "my son" word and Dawn deflects to morning); `b12_maya_finally_says_it` (adjacent_eot, Maya finally tells Amar half of his parentage — "half of you is from this side of the sea" — fits her promise from the ship); `b12_dawn_at_the_gate` (before_victory, Dawn welcomes squad at the inner gate; the "eleven years, Amar" line). **Spoils:** unchanged from existing scaffold (3 elixirs + 1 royal lens). **Music:** new `MUSIC.grudeBattle1` track (`audio/GrudeBattle1.mp3`) registered. **Two new arcs:** `before_ravage` — fourteen-month crossing montage (Maya teaches Amar Grude political pamphlets; Leo + dactyl finally settle by week 7; Amar carves a single word into the underside of Lucian's wooden practice sword; Maya breaks her own rule in month 11 and sets a letter from Amar's mother on the table) — closes on the alarm bell at Grude's customs platform. `post_ravage` — squad reaches Dawn's safe house, Ndara apologizes for the mountain village, Dawn gives the full colony-truth speech ("Anthros is a colony. Grude is the empire. Your father knew."), drops the bombshell that Lucian had a brother in Grude (Aren) who has a letter from Lucian written before the bolt. Family-tie reveal explicitly held back to B14. post_cliffs.next rerouted from `credits` to `story:before_ravage`; new slice end is `post_ravage` → credits. `EndScene.isFinalPlayable` gate moved from `b11_cliffs` to `b12_ravage`. RosterScene's ACTIVE_ROSTER `b12_ravage: ["amar", "ning", "maya", "leo"]`. README + index.html copy bumped from "11 battles" → "12 battles." |
...
| 2026-05 | B13 (Madame Dawn's Rebellion) authored — Rose dies for the captain | Heart chapter built on top of the B12 Grude arrival. Madame Dawn's lieutenant Rose is loaned to the squad for one fight — a four-block strike at the Grude plaza meant to convince the wider city that a rebellion is real — and pays for it before the chapter closes. **New unit:** `PLAYERS.rose` (shinobi, L13, hp 36 / power 12 / armor 4 / speed 14 / movement 5; weapon sword; new `G_ROSE` growth table tilted toward speed). New `rose` portrait id wired through `PortraitId` (beats.ts) + `PORTRAIT_IDS` (manifest.ts); new `PLAYER_PALETTES.rose` (oxide-green leather + rose-pink accent). Rose appears only in B13 — she does not carry forward into the post-B13 roster. **New enemy:** `ENEMIES.royalCaptain` ("Captain") — boss-tagged, sword, `holdPositionUntil { allyCount: 2 }`. Reuses the royal_guard portrait as a stand-in. **Map:** new 14×11 plaza (`dawnRebellionMap`) — five-slot squad spawn at south rows 9-10, marble podium with the Captain at row 0 centre, two crossbow archers on the podium row 1, two guards at the north benches at the corners, a sixth guard advancing through the dry fountain at row 7. New `Pl` tile alias for the podium pillars. **Victory:** `defeatUnit("royal_captain", { label: "Defeat the Captain" })` — narratively the rebellion is "real" the moment the city sees the empire's officer drop. **Mid-battle dialogues (4):** `b13_rose_brief` (round 1, Rose's tactical brief — east-side cohort under Maya's banner, the four-block timing window, "this isn't a sweep, it's a statement"); `b13_rose_amar_brief` (adjacent_eot Amar/Rose, Rose tells Amar she ran a bandit cohort under Madame Dawn for six years and "you don't have to like me — just stand on the line I draw"); `b13_amar_drops_captain` (ally_killed_target Amar/royal_captain, Rose's approval beat — "good. Now the city sees it."); `b13_rose_falls` (before_victory, the death scene — back-door volley, four crossbow bolts, Rose throws her shoulder under the high shaft meant for Madame Dawn, takes it through the chest; Dawn kneels in the blood). **Spoils:** 2 elixirs + 1 fang (the fang is Rose's — "memorial" loot). **Two new arcs:** `before_dawn_rebellion` — safe-house common-room briefing where Rose is introduced (knife on the table, Maya recognises her from the bandit years, Dawn's quiet "she fights with us tonight"); `post_dawn_rebellion` — Rose is buried under the lemon tree behind the safe house, Dawn sits with her in silence, Amar joins her, Dawn promises the parentage conversation tomorrow. `post_ravage.next` rerouted from `credits` to `story:before_dawn_rebellion`; new slice end is `post_dawn_rebellion` → credits. `EndScene.isFinalPlayable` gate moved from `b11_cliffs` to `b13_dawn_rebellion`. RosterScene's ACTIVE_ROSTER `b13_dawn_rebellion: ["amar", "ning", "maya", "leo", "rose"]` — Rose visible in the pre-battle roster card for B13 only. README + index.html copy bumped from "12 battles" → "13 battles." Tonally B13 is the squad's first lesson that the second half costs people too — they survived Lucian's death across the crossing, and now Dawn loses someone in front of them on day one of the rebellion. |
| 2026-05 | B14 (The Origin) authored — the parentage reveal | The reveal chapter. **New arc `before_origin`** is the conversation Dawn promised at the close of post_dawn_rebellion: in her study, with Maya + Ndara present, she tells Amar she is his mother and King Archbold of Grude is his father — he is the trueborn child of the rebellion's leader and the empire it exists to break. The arc deliberately reconciles a continuity seam: `before_ravage` had Maya tell Amar his father "died before you were born," so before_origin has Dawn explicitly own that as a cover story she fed her own officer (a sentence never spoken aloud can't be tortured out of anyone) — which both fixes the contradiction and characterises Dawn's information control. The study conversation is cut off mid-sentence by the candle-maker's far-watch signal: Archbold's household guard has found the safe house. **Battle:** the empire's first retrieval attempt — they want the heir alive, which recontextualises the fight (every prior royal officer wanted Amar dead; this one has orders to take him breathing). **New unit:** `ENEMIES.imperialKnight` ("Lord Castor", boss, L15, hp 64 / power 16 / armor 9 — highest-statline officer the squad has faced; household elite, not provincial garrison; `holdPositionUntil { allyCount: 2 }`, reuses the royal_guard stand-in portrait + archbold palette). **New map:** `originMap` — a 13×11 Grude inner-district street outside the safe house; squad spawns south at the marble doorstep, Castor's detail enters north; two ranks of market barricades + street well-heads (pillars) + a full-width junction at row 5 for flanking. **Victory:** `defeatUnit("imperial_knight", { label: "Defeat Lord Castor" })`. **Mid-battle dialogues (3):** `b14_castor_arrival` (round 1 — Castor states the retrieval order; Maya reads the "unharmed" as proof the study changed everything); `b14_amar_castor` (adjacent_eot — Castor, a 20-year household-guard veteran, frames Amar's heritage as a homecoming rather than a threat — the first voice to do so); `b14_castor_falls` (before_victory — Castor's disciplined withdrawal, "tonight goes in a report, not a grave"). **`royal_guard` added to `PortraitId`** so a stand-in-portrait officer can be a dialogue speaker (B12 Volos / B13 Captain never spoke; Castor does). **New arc `post_origin`** lands the chapter's thesis — "everything you have done was only ever half of it" — and seeds B15 with Maya, rattled that Dawn ran a cover story past her for eleven years, starting to watch the people who watch the squad. `post_dawn_rebellion.next` rerouted `credits` → `story:before_origin`; new slice end `post_origin` → credits. `EndScene.POST_ARC` + `isFinalPlayable` moved to `b14_origin`; `RosterScene` ACTIVE_ROSTER `b14_origin: ["amar","ning","maya","leo"]`. README + index.html bumped "13 battles" → "14 battles." |
| 2026-05 | B15 (A Coup Within a Coup) authored — the mole inside Dawn's house | The leak chapter. **New arc `before_inner_coup`** pays off the thread Maya started at the end of post_origin: she lays three months of supply manifests across the study floor and finds the one hand that touched every message — Quartermaster Coyne, Dawn's own Grude supply officer, the leak that put Castor's detail on the safe house at B14. Ndara reaches the same name an hour earlier, goes to face Coyne alone, and is found in the courtyard alive but not waking — a coma, not a grave, so she stays available for later chapters. **Battle:** the squad corners Coyne in the candle-maker's courtyard (the same courtyard where Rose is buried). **New unit:** `ENEMIES.turncoat` ("Quartermaster Coyne", boss, L14, hp 48 / power 13 / armor 6 / speed 13 — an intelligence officer's statline, fast + fragile rather than a knight; `holdPositionUntil`, royal_guard stand-in portrait + archbold palette). **New map:** `courtyardMap` — a 12×10 fully-enclosed courtyard, walls on all four sides with only two gaps (the shop door south = squad spawn, the back gate north = Coyne's intended escape); the lemon tree over Rose's grave (new `Lt` = cobblestone+tree alias) breaks the centre line, a well + two crate-barricade ranks give cover. **Enemies:** Coyne + 2 turncoat rebels (bandit factories — people his coin bought) + 2 imperial agents (royal factories — the empire muscle he smuggled in); the mix tells the betrayal's shape without a line of dialogue. **Victory:** `defeatUnit("turncoat", { label: "Defeat Quartermaster Coyne" })`. **Mid-battle dialogues (3):** `b15_coyne_exposed` (round 1 — Coyne doesn't deny it, he justifies it: "she spends people like coin and calls it arithmetic"); `b15_amar_coyne` (adjacent_eot — Coyne's real weapon is the squad's doubt, aimed at Amar: "whose arithmetic will you be doing?"); `b15_coyne_falls` (before_victory — Coyne dies six strides short of the gate, still counting). Coyne speaks portrait-less (`{ speaker, body }`, no portraitId — the established Lieutenant/Runner/Courier pattern) so no portrait asset was needed. **New arc `post_inner_coup`** is the hardening: betrayal from inside her own walls is what finally changes Madame Dawn — she stops asking the squad to follow and starts telling them, and the arc closes seeding B16 (she will demand Amar take the Anthros throne as the rebellion's open heir). Khione's prognosis for Ndara is honestly uncertain (wake tomorrow / wake in a season / never). `post_origin.next` rerouted `credits` → `story:before_inner_coup`; new slice end `post_inner_coup` → credits. `EndScene.POST_ARC` + `isFinalPlayable` moved to `b15_inner_coup`; `RosterScene` ACTIVE_ROSTER `b15_inner_coup: ["amar","ning","maya","leo"]` (Ndara comatose, off the field). README + index.html bumped "14 battles" → "15 battles." |
| 2026-05 | B16 (Dawn's Proposal) authored — the throne offer + the King's Knife | The proposal chapter. **New arc `before_proposal`** is the conversation Dawn promised: she asks Amar to claim the Anthros throne as the rebellion's open heir once Archbold falls — a colony that kills its emperor needs a face the moment the empire's is gone, and Amar is the only trueborn claimant alive. Amar answers "not yet" (not no, not yes — he won't decide on half a picture), which Dawn accepts; she sends the squad on a night errand across the river. **Battle:** the empire's answer arrives — King Archbold has decided a living heir is worse than a dead scandal and switches from *retrieve* (B14) to *kill*. The shift is the chapter: Amar decides what kind of son he is while his father decides the same about being a father. **New unit:** `ENEMIES.kingsKnife` ("Wren, the King's Knife", boss, L16, hp 46 / power 15 / armor 5 / speed 16 — an assassin's statline: the fastest unit in the game, high power, fragile; **no** `holdPositionUntil` — unlike the garrison bosses she rushes straight for Amar). **New map:** `bridgeMap` — a 14×9 stone bridge over the Grude river; `Wb` parapets wall the north + south edges so the deck is the whole playable span; stalled market carts (`Bd`) + bridge lamps (`Tr`) are the only cover. Squad enters mid-west; Wren's main force holds the east end while two hired knives drop in behind from the west — a two-sided pinch with no flank but along the deck. **Victory:** `defeatUnit("kings_knife", { label: "Defeat Wren" })`. **Mid-battle dialogues (3):** `b16_wren_ambush` (round 1 — Wren names the new kill order; Maya reads the tactics: drop Wren and the contract dissolves); `b16_amar_wren` (adjacent_eot — Wren, a professional not a believer, on the empire's one real lesson: "the only question is whether you read the list before you sign it"); `b16_wren_falls` (before_victory — Wren dies unsurprised, warns Amar to stop standing in the open). Wren speaks via the `royal_guard` stand-in portrait. **New arc `post_proposal`** lands the empire's new posture and plants B17: Khione — who has sat nights with the comatose Ndara — tells Amar there is a part of the story Dawn has told no one, the part Kian was reaching for on the cliff, and that Amar should hear it "on the water, where her walls are thinner" before he answers about any throne. `post_inner_coup.next` rerouted `credits` → `story:before_proposal`; new slice end `post_proposal` → credits. `EndScene.POST_ARC` + `isFinalPlayable` moved to `b16_proposal`; `RosterScene` ACTIVE_ROSTER `b16_proposal: ["amar","ning","maya","leo"]`. README + index.html bumped "15 battles" → "16 battles." |
| 2026-05 | B17 (Dawn's Lie) authored — the break with Dawn; the Grude arc's payload | The chapter the whole B14-B17 ramp was built toward. **New arc `before_lie`** is Khione's revelation, taken up on her offer from post_proposal ("come find me on the water"): Madame Dawn's rebellion was never a plan to *free* Anthros — it is a plan to *spend* it. A trueborn heir openly on the colony's throne is a provocation Archbold must answer by burning Anthros to its roots, and a colony visibly burning is the one thing Dawn believes will finally turn Grude against its own crown. Amar is the spark. Kian's cliff warning (B11) is confirmed — right, and only kindly under-sized. The squad resolves to break with Dawn and leave Grude. **Battle:** the squad runs for Khione's ship; Marshal Othren's loyalists — true believers in the plan, not traitors — form a line across the quay to stop the rebellion's lynchpin from walking. **New unit:** `ENEMIES.dawnLoyalist` ("Marshal Othren", boss, L15, hp 58 / power 14 / armor 9, spear — a heavy defensive anchor; carries the *bandit* palette, not archbold, because he is one of Dawn's own; `holdPositionUntil`). **New map:** `quayMap` — a 13×10 Grude harbour quay; squad enters north from the city, the ship's gangway (`Wo` wood, from the cliffs map) is the south escape; warehouse walls + stacked cargo + dock lamps for cover. **Victory:** `anyOf(escapeToTile gangway, routEnemies)` — mirrors B10's escape framing. Enemies are all bandit-factory units — Dawn's rebellion rank-and-file have been "bandits" mechanically since B3, so the line the squad breaks is built from them, deliberately *not* the royal factories: this fight is against people who believe. **Mid-battle dialogues (3):** `b17_othren_line` (round 1 — Othren, not cruel, blocks the way); `b17_amar_othren` (adjacent_eot — the lie confirmed from the believer's side: "I have done that arithmetic ten thousand times and I sleep well"); `b17_break_through` (before_victory — Othren doesn't give chase; "she'll let you go… you are not escaping her arithmetic, you are just becoming a different line of it"). **New arc `post_lie`** is the farewell: Dawn comes alone to the emptied dock, does not deny the plan and does not beg, reveals she *let* the squad reach the ship (the quay watch, Othren, Khione — all hers), and lets her son go. "She loves you. She lied. Both are true." The slice now ends on Amar in open water with the first genuinely unwritten choice of his life ahead — the Seven Paths (B18). `post_proposal.next` rerouted `credits` → `story:before_lie`; new slice end `post_lie` → credits. `EndScene.POST_ARC` + `isFinalPlayable` moved to `b17_lie`; `RosterScene` ACTIVE_ROSTER `b17_lie` entry. README + index.html bumped "16 battles" → "17 battles." |
...
- **Maps + units for B12-B30** — every battle now has a `rewards` field (B1-B11 playable + B12-B30 scaffolded), but B12-B30 still need maps, unit rosters, victory conditions, and arc beats authored before they go playable.
