...
import { FAMILY_BODY, FAMILY_DISPLAY, GAME_HEIGHT, GAME_WIDTH } from "../util/constants";
...
export class TitleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // Offered only when some save has actually passed the Seven Paths
    // fork — there is nothing to sail back to otherwise, and a button
    // that opens onto "you have no eligible saves" is worse than no
    // button. Deleting the last qualifying save removes it again.
    if (slotsPastThePathFork().length > 0) {
