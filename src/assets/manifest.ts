...
// Streamed assets live at STABLE urls with a day of browser cache
// (vercel.json) — so a re-dropped file kept serving stale art from
// returning players' caches for up to a day. Every loader call site
// routes its url through here: the content hash (generated by
// scripts/hashAssets.mjs at build time) rides along as ?v=, so changed
// bytes mean a changed url (instant refresh) while unchanged files keep
// their exact old url and stay cache-warm. Unhashed paths (a registered
// file that doesn't exist yet) pass through bare and 404 harmlessly,
// same as before.
export const versionedPath = (path: string): string => {
